...
var fr = new FileReader()
    fr.addEventListener("load", () => {
        document.getElementById("bartxt").value += "<img:"+fr.result+"";
        // loadScreenRemove(0)
    });
    fr.addEventListener("error", () => {jnhknmm
...
document.getElementById("fileupload").addEventListener("change", () => {
    ...
    fr.readAsDataURL(document.getElementById("fileupload").files[0])
});

var sentMessages = []
var sentMessagesIndex = 0
var connected = false
var muspitch = 1
...
function popup(type, arg = "") {
    if (type == "close"){
        ...
        if (type == "roomcClose"){
            ...
            var pdiv = document.createElement("div")
            pdiv.className = "link"
                var name = document.createElement("iframe")
                name.src = arg
                pdiv.append(name)
        ...
        if (!connected){
            ...
            document.body.addEventListener("keyup", (e) => {
                if (e.key == "ArrowUp") {
                    ...
                    document.getElementById("bartxt").value = sentMessages[sentMessagesIndex]
                }
                if (e.key == "ArrowDown") {
                    sentMessagesIndex += 1
...
function sendMessage(){
    ...
    sentMessages.push(document.getElementById("bartxt").value)
    ...
    if (document.getElementById("chatdiv")){
        ...
        msgbar.id = "msgbar"
            ...
            btn.className = "upload"
                ...
                // img.height = "100%"
                btn.append(img)
            btn.setAttribute("onclick", 'uploadImg()')
...
function displayMessage(type, message, username, charInfo){
    ...
    if (type == "nyou"){
        ...
        var nameplate = document.createElement("div")
            ...
            nameplate.append(uname)
            var pfp = document.createElement("img")
                pfp.className = "pfp"
                pfp.src = miiAPI + charInfo.data
                pfp.style = "background-color: " + charInfo.background
            nameplate.append(pfp)
...
function stylizeText(text){
  ...
  for (let i = 0; i < t.length; i++) {
    ...
    if (element.includes("http:") || element.includes("https:")){
      console.log(element)
      final = `<a href="#" onclick="popup('URL', '${element}')">${element}</a>`
    }
...
async function checkForUpdates(){
    var api = await fetch("https://api.github.com/repos/windnedr/harmii/releases")
    alert(api.body)
    var data = JSON.parse(api.blob)
    data[0].published_at
    popup("update",data[0].body)
}
